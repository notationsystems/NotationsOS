...
export interface ReleaseManifestV0 {
  ...
  coverage: string;
  sources: Array<{ sourceId: string; licence: string; permittedUses: string[]; nonUse: string[]; redistribution: string; attributionRequired: boolean }>;
...
export function buildReleaseManifest(corpus: Corpus, release: CorpusRelease): ReleaseManifestV0 {
  return {
    ...
    sources: release.sources.map((s) => ({ sourceId: s.sourceId, licence: s.licence, permittedUses: s.permittedUses, nonUse: s.nonUse, redistribution: s.redistribution, attributionRequired: s.attributionRequired })),
