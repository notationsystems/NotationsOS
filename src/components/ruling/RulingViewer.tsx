    ...
    <div className="flex flex-col min-h-full" data-testid="ruling-viewer">
      ...
      <div className="max-w-[960px] w-full mx-auto p-3 sm:p-5 flex flex-col gap-5">
        ...
        <dl className="grid gap-x-6 gap-y-3 grid-cols-1 sm:grid-cols-2 m-0 surface p-4">
          ...
          <div><dt className="label-sm">Corpus release</dt><dd className="m-0"><Link href={`/releases/${encodeURIComponent(ruling.corpus.releaseId)}`} className="id" style={{ color: 'var(--info)' }}>{ruling.corpus.releaseId}</Link></dd><dd className="m-0 text-[12px]" style={{ color: 'var(--text-muted)' }}>build <span className="id">{ruling.corpus.buildId}</span> · known by <span className="ts">{fmtUtc(ruling.corpus.knownAt)}</span></dd></div>
