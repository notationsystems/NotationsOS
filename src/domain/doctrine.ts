...
export const FABRICS: readonly Fabric[] = [
  ...
  { id: 'state', order: 3, title: 'State fabric', transforms: 'Candidate → validation → version', inThisRepository: 'Absent as a kernel. Admission is a separate act that nothing here performs; releases are committed fixtures with certification, manifests, corrections and recalls. The deterministic state kernel is specified outside this repository.', presence: 'ABSENT' },
  ...
  { id: 'projection', order: 5, title: 'Projection fabric', transforms: 'State or inquiry → human- and machine-operable representation', inThisRepository: 'The feed API, the MCP tools, the stream and every page are projections of committed releases. The ProjectionSpec and its router exist and are tested; kepler.gl, CesiumJS and Three.js are not installed.', presence: 'FIXTURE', where: '/api' },
];
...
export const INFORMATION_STATES: readonly InformationState[] = [
  ...
  { id: 'INQUIRY', symbol: 'I', title: 'Inquiry', meaning: 'What one investigation is currently manipulating: selected evidence, hypotheses, calculations, candidate deltas. Allowed to be wrong.', invariants: ['exploratory', 'mutable', 'never a source of truth', 'promotion crosses validation'], inThisRepository: 'The intake draft, saved unevaluated with submission as an intent; candidates on the rail. No general inquiry workspace yet.', where: '/cases/new' },
];
...
export const DOCTRINE: readonly DoctrineRule[] = [
  ...
  { n: 5, rule: 'Projection never mutates its source.', meaning: 'A map, a globe, a graph, a feed or a page reads state. It has no path back into it.', enforcedHere: 'Feed payloads and MCP results are pure functions of committed releases: the fixture corpus is identical before and after every projection; the projection router is pure and its plan states mutatesSource: false.', where: '/api', tests: ['src/architecture.test.ts', 'src/domain/projection.test.ts'] },
  { n: 6, rule: 'Identity survives representation changes.', meaning: 'A projection changes how a thing is shown, never what it is. Visual adjacency is not a relation.', enforcedHere: 'A record keeps the same notation:// identity in the records feed, the as-of answer and every MCP result; a projection plan carries its referents unchanged and derives no relations.', where: '/stream', tests: ['src/architecture.test.ts', 'src/domain/projection.test.ts'] },
...
export const PROJECTION_ENGINE_PRESENCE: Record<ProjectionEngine, Presence> = { 'kepler.gl': 'ABSENT', CesiumJS: 'ABSENT', 'Three.js': 'ABSENT', table: 'PRESENT' };
