...
export const FABRICS: readonly Fabric[] = [
  ...
  { id: 'compute', order: 4, title: 'Compute and decision fabric', transforms: 'State → model → result → decision', inThisRepository: 'Bounded local instruments run: the pinned GAT IFC audit, recorded-observation replay, and a scalar linear-Gaussian benchmark with exact evidence and model references. The benchmark demonstration is synthetic, not field validation or full sensor fusion. The ruling workbench remains fixture-backed. Managed customer workloads, trained neural models and automatic canonical admission remain absent.', presence: 'PRESENT' },
