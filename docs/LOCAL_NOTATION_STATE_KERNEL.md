...
Successful responses contain the notation state, `savedVersion`, `savedDigest`, `enabled`, `persistence`, `mode: "LOCAL_DEVELOPMENT"` and `canonicalAdmission: false`. Preview changes the returned command revision, not the saved version. Errors are structured `{ error: { code, message } }` responses; source payloads, native stderr and local paths are not returned.
...
The browser test checks stable ID/content after a full page reload, reconstructed redo availability, viewport overflow, page errors and serious/critical accessibility findings. Screenshots are written under `test-results/`. This is local implementation verification, not independent verification or a completed customer pilot.
