...
- Projection is a contract before it is an engine. `domain/projection.ts` defines `ProjectionSpec` and a pure, total router to kepler.gl, CesiumJS, Three.js or the table by coordinate semantics and intent; a plan carries its referents unchanged and derives no relation. None of the three engines is installed; the feed, the tools, the stream and the pages are the projections that exist, and `src/architecture.test.ts` proves they leave the corpus untouched and identities intact. Full text: [Architecture](ARCHITECTURE.md).
