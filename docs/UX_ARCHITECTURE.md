...
The `/notations` workspace is a separate local authored-state consumer of the [Rust state kernel](LOCAL_NOTATION_STATE_KERNEL.md). The browser sends closed command batches, not replacement state. Create/update/relations and undo/redo are previewed by Rust; only Save persists an immutable version. Saved version, draft command revision, pending command count and remaining lifetime command/save capacity are shown separately. A root client provider retains drafts, selection and in-flight requests across internal route changes, keeping the original saved base version; it is not durable browser storage. Failed requests preserve drafts; reload discards them only after explicit confirmation and successful readback. At command capacity a pending batch remains saveable; at saved-version capacity further previews and new saves are blocked. The screen is disabled by default and reads local data only through the guarded client API, never at server-render time. It creates no evidence, ruling, corpus admission, release or Bevy world.
