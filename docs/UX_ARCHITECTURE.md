...
| `/candidates` | the candidate-production rail before admission: acquisitions with INGEST decisions, bytes and receipts; normalization runs with their own DERIVE decision, one UNRESOLVED/UNADMITTED candidate and one quarantine; the time-bounded candidate build with per-member cutoff checks and build-time DERIVE; the three refused steps; the boundary statement; how to reproduce | `components/production/CandidatePipeline.tsx` |
...
The `/notations` workspace is a separate local authored-state consumer of the [Rust state kernel](LOCAL_NOTATION_STATE_KERNEL.md); its drafts, states, conflict recovery, capacity view and the fixture evidence-reference panel are specified in [Notation workspace](NOTATION_WORKSPACE.md). The browser sends closed command batches, not replacement state. Create/update/relations and undo/redo are previewed by Rust; only Save persists an immutable version. Saved version, draft command revision and pending command count are shown separately. Failed requests preserve drafts; reload discards them only after explicit confirmation and successful readback. The screen is disabled by default and reads local data only through the guarded client API, never at server-render time. It creates no evidence, ruling, corpus admission, release or Bevy world.
