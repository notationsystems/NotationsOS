...
| The architecture carried forward: five fabrics, three states of information, seven rules of doctrine, verification tiers, the projection triad | `src/domain/doctrine.ts`, `src/domain/projection.ts`, `docs/ARCHITECTURE.md`; `/product`; `src/domain/doctrine.test.ts` keeps every named test present; `src/architecture.test.ts` proves rules 5, 6 and 7 structurally | Stated as data with presence flags; the state kernel and the three projection engines absent, and stated so |
