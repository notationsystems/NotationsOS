...
The local evidence rail is another shared production facility: it evaluates a declared source policy, captures local bytes with a storage receipt and reopens the acquisition for integrity checks. Its authorization basis is an operator declaration. It creates no normalized or canonical domain records and activates no release. The implementation is documented in [Local evidence intake](LOCAL_EVIDENCE_INTAKE.md).
...
| Capture authorized local source material and retain its bytes and receipt | `src/data-os/local-intake.ts`, `source-policy.ts`, `evidence-capture.ts`, `file-object-store.ts`, `local-files.ts`; `npm run evidence` | Local implementation: exact INTERNAL INGEST evaluated against operator-declared policy; create-only content-addressed files and acquisition metadata; readback and reinspection. No independent authorization, normalization, canonical admission, physical WORM or production storage claim |
