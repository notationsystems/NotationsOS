...
It does not normalize the file, create canonical domain records, build a corpus, activate a release or send data to a customer. The source registration is an operator declaration. Its evaluation does not independently establish that the operator possesses the stated source rights.
...
Policy windows are half-open: the registration is inactive at `effectiveUntil` and afterward. Dates require an explicit timezone and a valid calendar instant. Storage cannot precede capture. A grant for ingestion does not grant indexing, publication, redistribution, customer delivery, model training or any other use. Retention declarations are validated as policy fields; no physical retention lock, expiry deletion or automatic cleanup is implemented.
...
| `src/source-connector-workflow.js` | A connector binds one source registration and versioned media/schema contract. A verified quarantine decision still blocks normalization. Transport and schema adjudication remain future integration work here. |
| `src/evidence-normalization-workflow.js` | Preserve exact source, acquisition, evidence, storage-receipt and method references; preserve missingness and unobserved time. Provenance verification alone does not establish extraction execution or field accuracy. Normalization is not implemented in this increment. |
