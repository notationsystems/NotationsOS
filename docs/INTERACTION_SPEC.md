...
## Notation workspace (C-note)

`/notations` keeps three things distinct on screen: unapplied form text (field count, an "Unapplied edit" marker), kernel-validated commands that are not saved (count and draft revision, "Save may still conflict"), and the saved local version (number and digest). Drafts survive internal navigation, browser back and reload in the tab; a same-origin link to another page while work is unsaved opens an alert dialog with Stay (focused; Escape), Leave and keep drafts, and Discard drafts and leave. A refused preview or a failed save keeps the drafts. A version conflict opens a panel that names both versions, lists the pending commands, offers Copy drafts as JSON with a selectable fallback, and leaves reload deliberate. The capacity meter shows used / limit / remaining per dimension, warns at ninety percent and at each limit disables the affected controls and states the recovery. A preview says it is not saved and that another save may still win. The evidence-reference panel below is a marked fixture with one example of every resolution state and attachment DISABLED. C-note: `src/components/notations/*.test.tsx` over a mocked API; `tests/e2e/notation-state.spec.ts` over the real kernel on desktop and mobile.

## Information product (C-prod)

`/products` states the first information product as data and shows the corpus's coverage of it: each field's evidence requirement with how many released records are within it (the test requires all), the releases and their cutoffs, the prohibited purposes, and correction as the same question answered at two knowledge times with the earlier record superseded, never erased. The ten-question delivered-record contract is shown with the fields that carry each answer; `deliveredRecord.test.ts` holds every delivered record on both projections and in every release to all ten. The acceptance target is shown step by step with what this repository reaches and what it does not.

## Candidates (C-cand)

`/candidates` shows the local production rail before admission. Acquisition, normalization and candidate build are three separate steps with three separate source-use decisions (INGEST at capture, DERIVE at normalization, DERIVE again at build time), and the page shows each decision with its reasons and instant. A normalization is either `NORMALIZED` with one `UNADMITTED` candidate whose identity is `UNRESOLVED` and whose `canonicalId` is `null`, or `QUARANTINED` with no candidate and the adapter's reason. A build lists its members with the knowledge-time check `knownAt ≤ cutoff ≤ builtAt` per member. Refused steps are shown with the rail's own message and its meaning; a refusal writes nothing. The boundary statement at the top says what none of this is: canonical admission, inventory, a release, or anything the feed or the tools can return. C-cand in `src/fixtures/production/demo.contract.test.ts` reproduces the whole demonstration through the rails and asserts the separation; the smoke test checks the states on the page and that the records feed contains no candidate identifier.
